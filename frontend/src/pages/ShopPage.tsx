...
export const ShopPage: React.FC = () => {
  ...
  const [celebration, setCelebration] = useState<{
    ...
    subtitle: string;
    gemsEarned: number;
    variant: 'purchase' | 'streak_freeze';
  } | null>(null);
  ...
  const handlePurchase = async (item: ShopItem) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show success celebration instead of toast
        let celebrationData: any = {
          isOpen: true,
          title: "Purchase Successful!",
          subtitle: `${item.name} purchased successfully!`,
          gemsEarned: 0,
          variant: 'purchase'
        };

        if (result.effect.type === 'refill_hearts') {
          celebrationData.subtitle = `Hearts refilled! You now have ${result.effect.hearts} hearts.`;
        } else if (result.effect.type === 'double_xp') {
          celebrationData.subtitle = `Double XP activated! Earn 2x XP for the next ${result.effect.duration_minutes} minutes.`;
        } else if (result.effect.type === 'streak_freeze') {
          celebrationData.title = "Streak Freeze Activated!";
          celebrationData.subtitle = `Your streak is protected for ${result.effect.days} day.`;
          celebrationData.variant = 'streak_freeze';
        }
